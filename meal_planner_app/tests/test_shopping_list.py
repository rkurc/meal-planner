...
class TestShoppingList(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.recipe4_no_ing = crud.create_recipe(name="Water", instructions="Pour water.")


        self.meal_plan1 = crud.create_meal_plan(name="Breakfast Week")
        crud.add_recipe_to_meal_plan(self.meal_plan1.id, self.recipe1.id)
        crud.add_recipe_to_meal_plan(self.meal_plan1.id, self.recipe2.id)

        self.meal_plan2_complex = crud.create_meal_plan(name="Full Menu")
        crud.add_recipe_to_meal_plan(self.meal_plan2_complex.id, self.recipe1.id) # Pancakes
        crud.add_recipe_to_meal_plan(self.meal_plan2_complex.id, self.recipe2.id) # Omelette
        crud.add_recipe_to_meal_plan(self.meal_plan2_complex.id, self.recipe3.id) # Milkshake

        self.meal_plan_empty_recipes = crud.create_meal_plan(name="Empty Recipes Plan")
        crud.add_recipe_to_meal_plan(self.meal_plan_empty_recipes.id, self.recipe4_no_ing.id)
    ...
    def test_generate_shopping_list_basic_aggregation(self):
        ...
        self.assertEqual(len(shopping_list), 6) # Flour, Sugar (cup), Egg, Cheese, Salt, Pepper

    def test_generate_shopping_list_complex_aggregation(self):
        shopping_list = crud.generate_shopping_list(self.meal_plan2_complex.id)
        self.assertIsNotNone(shopping_list)
    ...
    def test_shopping_list_with_mixed_numeric_non_numeric(self):
        ...
        crud.add_recipe_to_meal_plan(self.meal_plan1.id, recipe_extra.id)
        # Now meal_plan1 has:
        # R1: Flour, 2 cup
        # R2: (no flour)
        ...
        self.assertIn('2.0', flour_cup['quantity']) # Original numeric becomes string in list
        self.assertIn('a bit more', flour_cup['quantity'])


    def test_shopping_list_empty_meal_plan(self):
        shopping_list = crud.generate_shopping_list(self.meal_plan_no_recipes.id)
        self.assertIsNotNone(shopping_list)
        self.assertEqual(len(shopping_list), 0)

    def test_shopping_list_meal_plan_with_empty_recipes(self):
        shopping_list = crud.generate_shopping_list(self.meal_plan_empty_recipes.id)
        self.assertIsNotNone(shopping_list)
        self.assertEqual(len(shopping_list), 0)
