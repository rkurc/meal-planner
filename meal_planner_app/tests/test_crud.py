"""
Tests for the CRUD operations in crud.py.
"""
...
class TestRecipeCRUD(unittest.TestCase):
    ...
    def test_create_and_get_recipe(self):
        ...
        self.assertIsNotNone(recipe1.recipe_id)
        self.assertEqual(recipe1.name, "Pancakes")

        retrieved_recipe = crud.get_recipe(recipe1.recipe_id)
        self.assertIsNotNone(retrieved_recipe)
        self.assertEqual(retrieved_recipe.recipe_id, recipe1.recipe_id)
    ...
    def test_update_recipe(self):
        ...
        further_updated_recipe = crud.update_recipe(
            recipe.recipe_id, source_url="http://example.com/soup"
        )
        self.assertIsNotNone(further_updated_recipe)
        self.assertEqual(further_updated_recipe.source_url, "http://example.com/soup")
        self.assertEqual(
            further_updated_recipe.name, updated_name
    ...
    def test_delete_recipe(self):
        ...
        self.assertTrue(crud.delete_recipe(recipe1.recipe_id))
        self.assertIsNone(crud.get_recipe(recipe1.recipe_id))
        self.assertIsNotNone(
            crud.get_recipe(recipe2.recipe_id)
        )  # Ensure other recipe is not deleted
...
class TestMealPlanCRUD(unittest.TestCase):
    """Tests for MealPlan CRUD operations."""
    ...
    def test_add_recipe_to_meal_plan(self):
        """Test adding recipes to a meal plan."""
        ...
        self.assertIsNone(
            crud.add_recipe_to_meal_plan(non_existent_mp_id, self.recipe1.recipe_id)
        )
    ...
    def test_remove_recipe_from_meal_plan(self):
        ...
        updated_mp = crud.remove_recipe_from_meal_plan(
        ...
        self.assertIsNone(
            crud.remove_recipe_from_meal_plan(uuid.uuid4(), self.recipe1.recipe_id)
        )
    ...
    def test_delete_meal_plan(self):
        ...
        self.assertTrue(crud.delete_meal_plan(mp1.meal_plan_id))
        self.assertIsNone(crud.get_meal_plan(mp1.meal_plan_id))
        self.assertIsNotNone(crud.get_meal_plan(mp2.meal_plan_id))
    ...
    def test_update_meal_plan(self):
        ...
        updated_mp_name = crud.update_meal_plan(mp.meal_plan_id, name="New Name")
        self.assertEqual(updated_mp_name.name, "New Name")
        self.assertIn(
            self.recipe1.recipe_id, updated_mp_name.recipe_ids
        )  # Recipes should be unchanged

        # Update only the recipes
        updated_mp_recipes = crud.update_meal_plan(
            mp.meal_plan_id, recipe_ids=[self.recipe2.recipe_id]
        )
        self.assertEqual(
            updated_mp_recipes.name, "New Name"
        )  # Name should be unchanged from previous step
        self.assertNotIn(self.recipe1.recipe_id, updated_mp_recipes.recipe_ids)
        self.assertIn(self.recipe2.recipe_id, updated_mp_recipes.recipe_ids)

        # Update both
        updated_mp_both = crud.update_meal_plan(
            mp.meal_plan_id, name="Final Name", recipe_ids=[]
        )
...
class TestRecipeSearch(unittest.TestCase):
    ...
    def test_search_by_full_name(self):
        """Test searching by a recipe's full name."""
        results = crud.search_recipes("Classic Pancakes")
        self.assertEqual(len(results), 1)
        self.assertEqual(results[0].recipe_id, self.recipe1.recipe_id)

    def test_search_by_partial_name(self):
        """Test searching by a partial, case-insensitive name."""
        results = crud.search_recipes("pancake")  # Case-insensitive
        self.assertEqual(len(results), 1)
        self.assertEqual(results[0].recipe_id, self.recipe1.recipe_id)

    def test_search_by_keyword_in_description(self):
        """Test searching by keywords in the recipe description."""
        ...
        self.assertEqual(results_spicy[0].recipe_id, self.recipe2.recipe_id)

    def test_search_by_ingredient_name(self):
        """Test searching by an ingredient name."""
        ...
        self.assertEqual(results_flour[0].recipe_id, self.recipe1.recipe_id)

    def test_search_multiple_results(self):
        """Test a search query that should return multiple recipes."""
        results = crud.search_recipes("egg")
        self.assertEqual(len(results), 2)  # Recipe1, Recipe4_no_desc (Boiled Egg)
        # R2 Chicken Soup does not have 'egg' in name/desc, and 'Chicken Breast'
        # is the ingredient.
        # Oh, I made a mistake in the test logic. My recipe1 and recipe4 have "Egg"
        # as an ingredient.
    ...
    def test_search_single_result(self):
        """Test a search query that should return a single, specific recipe."""
        results = crud.search_recipes("Chili Flakes")
        self.assertEqual(len(results), 1)
        self.assertEqual(results[0].recipe_id, self.recipe2.recipe_id)

    def test_search_no_results(self):
        """Test a search query that should return no results."""
    ...
    def test_search_query_matches_multiple_fields_in_same_recipe(self):
        """Test that a recipe is returned only once if the query matches multiple fields."""
        results = crud.search_recipes("pancakes")
        self.assertEqual(len(results), 1)
        self.assertEqual(results[0].recipe_id, self.recipe1.recipe_id)
    ...
    def test_search_recipe_with_no_description(self):
        """Test searching for a recipe that has no description."""
        results = crud.search_recipes("Boiled")  # Matches name of recipe4_no_desc
        self.assertEqual(len(results), 1)
        self.assertEqual(results[0].recipe_id, self.recipe4_no_desc.recipe_id)
        ...
        self.assertEqual(results_desc_only[0].recipe_id, self.recipe1.recipe_id)
    ...
    def test_search_with_ingredient_filter(self):
        """Test searching with both a query and an ingredient filter."""
    ...
    def test_search_with_ingredient_filter_no_match_in_query_results(self):
        """Test when the query finds recipes, but the ingredient filter matches none of them."""
    ...
    def test_search_with_ingredient_filter_query_empty(self):
        """Test searching with only an ingredient filter and no query."""
    ...
    def test_search_with_ingredient_filter_query_empty_no_results(self):
        """Test searching with an ingredient filter that matches no recipes."""
        # No query, filter by "NonExistentIngredient"
        results = crud.search_recipes(
            query="", filter_ingredient="NonExistentIngredient"
        )
        self.assertEqual(len(results), 0)

    def test_search_with_ingredient_filter_both_present(self):
        """Test a search where both the query and ingredient filter match the same recipe."""
    ...
    def test_search_with_ingredient_filter_case_insensitivity(self):
        """Test that the ingredient filter is case-insensitive."""
        # Search "classic" (matches R1), filter by "milk" (matches R1)
        results = crud.search_recipes(query="classic", filter_ingredient="milk")
        self.assertEqual(len(results), 1)
        self.assertEqual(results[0].recipe_id, self.recipe1.recipe_id)
    ...
    def test_search_with_ingredient_filter_empty_filter(self):
        """Test that an empty ingredient filter is ignored."""
        ...
        self.assertEqual(results_none[0].recipe_id, self.recipe2.recipe_id)

    def test_search_both_empty_query_and_filter(self):
        """Test that providing no query and no filter returns no results."""
