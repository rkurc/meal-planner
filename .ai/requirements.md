...
### 1.1 Automatic Recipe Discovery *(NOT IMPLEMENTED - FUTURE / DESIRED FEATURE)*

**Status note (2026-06-16):** No implementation, no UI, no backend logic, no dependencies for discovery/scraping/NLP. All FRs below are aspirational. Test cases TC-ARD-* marked future.
...
### 1.2 Recipe Management *(IMPLEMENTED - API + React + Legacy)*

**Status note:** Full CRUD implemented end-to-end (backend API, React components, Jinja2). All listed FRs satisfied.
...
### 1.3 Ingredient Management *(NOT IMPLEMENTED AS STANDALONE - FUTURE)*

**Status note:** No master list CRUD UI or endpoints (`/api/ingredients` etc. do not exist). FR-1.3.1-1.3.3 are not met. FR-1.3.4 (flexible units) *is* supported in embedded recipe ingredients (models + CRUD + shopping consolidation).
...
### 1.4 Meal Plan Management *(IMPLEMENTED - API + React + Legacy)*

**Status note:** Full implementation including adding recipes to plans and generating shopping lists from them.
...
### 1.5 Shopping List Generation *(IMPLEMENTED - API + React view/edit + Legacy + PDF legacy)*

**Status note:** Full implementation of generation, consolidation logic, persistence (separate shopping list resource), manual editing, and display (both UIs). PDF only in legacy for now.
...
*   **FR-1.6.2:** The traditional interface (Jinja2) shall provide access to all defined features. *(Currently true; legacy is complete.)*
*   **FR-1.6.3:** The modern interface (React @ `/ui/` or `/static/react_app/`) shall provide, at a minimum, the ability to display recipes. *(Exceeded: full CRUD for recipes, meal plans, shopping lists.)*

### 1.7 Future/Desired Features

*   **FR-1.7.1:** The system should provide an advanced search functionality to filter locally stored recipes. *(Not in React; legacy has basic.)*
*   **FR-1.7.2:** The system should allow exporting a shopping list or meal plan to a PDF document. *(Legacy only.)*
...
### 2.1 Performance
*   **NFR-2.1.1:** Web pages should load in a user's browser in under 3 seconds on a standard broadband connection. *(Not formally measured; dev env responsive.)*
*   **NFR-2.1.2:** Recipe search/extraction from a given URL should complete within 15 seconds. *(Future - no discovery yet.)*
*   **NFR-2.1.3:** The `/api/recipes` endpoint should respond with data in under 500ms for up to 1000 recipes. *(Plausible for current in-mem impl; not load-tested.)*

### 2.2 Usability
*   **NFR-2.2.1:** The user interface should be intuitive and require minimal training. *(Applies to current UIs.)*
*   **NFR-2.2.2:** The system should provide clear feedback to the user during the recipe extraction process (e.g., "Crawling page...", "Extracting ingredients...", "Success!"). *(Future - applies only if discovery implemented.)*

### 2.3 Reliability
*   **NFR-2.3.1:** The application should be available and operational 99.9% of the time.
*   **NFR-2.3.2:** The system should handle common user errors gracefully (e.g., invalid URL, non-recipe webpage). *(Current error handling in CRUD flows; discovery errors are future.)*

### 2.4 Maintainability
*   **NFR-2.4.1:** The codebase should be well-documented.
*   **NFR-2.4.2:** The project should follow a consistent coding style. *(Enforced via pre-commit: black + pylint for py; prettier for frontend.)*
*   **NFR-2.4.3:** The AI/extraction logic should be modular and separable from the core application logic to allow for independent updates and improvements. *(Future requirement.)*

### 2.5 AI/Extraction Accuracy
*   **NFR-2.5.1:** The AI extraction process shall correctly identify and extract ingredients and their quantities with at least 90% accuracy on a benchmark set of common recipe websites. *(Future / not applicable until discovery implemented.)*
